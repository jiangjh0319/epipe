    ...
    <section class="reimburse">
        ...
        <div class="content">
            ...
            <div v-for="(item,index) in dataObj.list" :key="index">
                <div class="reimburseCount">物品明细 {{index+1}}</div>
            ...
            <div class="styles infor" v-if="dataObj.materialReceiveRemarks">
                <div class="infor-box">
...
<script>
    ...
    export default {
        ...
        ...mapMutations(['change_man','approver_man']),
            refuse:function(){
                 this.$router.push({path:'/opinion',query:{id:this.dataObj.materialReceiveId,typeName:'material',applyType:21,color:'#f80'}})
            },
            ...
            deliverTo(){ //转交
                ...
                this.$router.push({path:'/imchoices',query:{id:this.dataObj.materialReceiveId,receiverIds:newCopy,careOf:true,typeName:'material',applyType:21,bgcolor:'#f80',auditerIds:newApprStr,num:1}})
            },
            approveBack(){ //退回
                 this.$router.push({path:'/approveBack',query:{id:this.dataObj.materialReceiveId,typeName:'material',applyType:21,color:'#f80'}})
            },
            consent:function(type){
                ...
                params={id:this.dataObj.materialReceiveId,receiverIds,auditerIds,receiverCompanyId,auditCompanyId,
                color:'#f80',applyType:21,typeName:'material',pageType:type,careOf:true,num:1}

                this.$router.push({path:url,query:params})
            },
            resubmit(){ //再次提交
                this.$router.replace({path:'/material',query:{materialId:this.dataObj.materialReceiveId,resubmit:1}})
            },
            urge(){ //催办
                this.isBackout = false;
                let that = this;
                this.axios.post('/work/audit'+this.Service.queryString({
                        applyId:this.dataObj.materialReceiveId,
                        type:6,
                        applyType:21,
                })).then(function(res){
                ...
                this.axios.post('/work/audit'+this.Service.queryString({
                    ...
                    applyType:21,
                        ...
                        if(res.data.h.code!=200){
                            ...
                            setTimeout(()=>{
                                window.location.href = "epipe://?&mark=materialDetails&_id="+that.dataObj.materialReceiveId+'&data='+JSON.stringify({text:1});;
                            },500)     
                        } 
                    })
            },
        ...
        mounted:function(){
            ...
            this.axios.get('/work/material/info?materialReceiveApplyId='+this.materialId+'&pushId='+pusthId).then((res)=>{
                ...
                that.title = that.dataObj.username+'的物品领用申请'
                ...
                // let arr=[];
                // that.accessory = that.accessoryFors(that.dataObj.accessory)
                // that.title = that.dataObj.username+'的物品领用'
