...
<script>
    ...
    export default {
        ...
        ...mapMutations(['change_man','approver_man']),
            refuse:function(){
                 this.$router.push({path:'/opinion',query:{id:this.dataObj.positionApplyId,typeName:'position',applyType:19,color:'#f80'}})
            },
            ...
            deliverTo(){ //转交
                ...
                this.$router.push({path:'/imchoices',query:{id:this.dataObj.positionApplyId,receiverIds:newCopy,careOf:true,typeName:'position',applyType:19,bgcolor:'#f80',auditerIds:newApprStr,num:1}})
            },
            approveBack(){ //退回
                 this.$router.push({path:'/approveBack',query:{id:this.dataObj.positionApplyId,typeName:'position',applyType:19,color:'#f80'}})
            },
            consent:function(type){
                ...
                params={id:this.dataObj.positionApplyId,receiverIds,auditerIds,receiverCompanyId,auditCompanyId,
                color:'#f80',applyType:19,typeName:'position',pageType:type,careOf:true,num:1}
            ...
            resubmit(){ //再次提交
                this.$router.replace({path:'/position',query:{positionId:this.dataObj.positionApplyId,resubmit:1}})
            },
            urge(){ //催办
                this.isBackout = false;
                let that = this;
                this.axios.post('/work/audit'+this.Service.queryString({
                    applyId:this.dataObj.positionApplyId,
                    type:6,
                    applyType:19,
                })).then(function(res){
                ...
                this.axios.post('/work/audit'+this.Service.queryString({
                    ...
                    applyType:19,
                        ...
                        if(res.data.h.code!=200){
                            ...
                            setTimeout(()=>{
                                window.location.href = "epipe://?&mark=positionDetails&_id="+that.dataObj.positionApplyId+'&data='+JSON.stringify({text:1});;
                            },500)     
                        } 
                    })
            },
        ...
        mounted:function(){
            ...
            this.axios.get('/work/changeposition/info?positionApplyId='+this.positionId+'&pushId='+pusthId).then(function(res){
                that.dataObj = res.data.b;
                let arr=[];
                that.accessory = that.accessoryFors(that.dataObj.accessory)
                that.title = that.dataObj.username+'的调岗申请'
