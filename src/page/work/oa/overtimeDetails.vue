...
<script>
    ...
    export default {
        ...
        ...mapMutations(['change_man','approver_man']),
            refuse:function(){
                 this.$router.push({path:'/opinion',query:{id:this.dataObj.overTimeApplyId,typeName:'overtime',applyType:18,color:'#fd545c'}})
            },
            ...
            deliverTo(){ //转交
                ...
                this.$router.push({path:'/imchoices',query:{id:this.dataObj.overTimeApplyId,receiverIds:newCopy,careOf:true,typeName:'overtime',applyType:18,bgcolor:'#fd545c',auditerIds:newApprStr,num:1}})
            },
            approveBack(){ //退回
                 this.$router.push({path:'/approveBack',query:{id:this.dataObj.overTimeApplyId,typeName:'overtime',applyType:18,color:'#fd545c'}})
            },
            consent:function(type){
                ...
                params={id:this.dataObj.overTimeApplyId,receiverIds,auditerIds,receiverCompanyId,auditCompanyId,
                color:'#fd545c',applyType:18,typeName:'overtime',pageType:type,careOf:true,num:1}
            ...
            resubmit(){ //再次提交
                this.$router.replace({path:'/overtime',query:{overtimeId:this.dataObj.overTimeApplyId,resubmit:1}})
            },
            urge(){ //催办
                this.isBackout = false;
                let that = this;
                this.axios.post('/work/audit'+this.Service.queryString({
                    applyId:this.dataObj.overTimeApplyId,
                    type:6,
                    applyType:18,
                })).then(function(res){
                ...
                this.axios.post('/work/audit'+this.Service.queryString({
                    ...
                    applyType:18,
                        ...
                        if(res.data.h.code!=200){
                            ...
                            setTimeout(()=>{
                                window.location.href = "epipe://?&mark=overtimeDetails&_id="+that.dataObj.overTimeApplyId+'&data='+JSON.stringify({text:1});;
                            },500)     
                        } 
                    })
            },
        ...
        mounted:function(){
            ...
            let that = this;
            this.overtimeId = this.$route.query.overtimeId;
            let pusthId = this.$route.query.pushId
            this.axios.get('/work/overtime/info?overTimeApplyId='+this.overtimeId+'&pushId='+pusthId).then(function(res){
                that.dataObj = res.data.b;
                let arr=[];
                that.accessory = that.accessoryFors(that.dataObj.accessory)
                that.title = that.dataObj.username+'的加班申请'
