    ...
    <section class="reimburse">
        ...
        <div class="content">
            ...
            <div class="styles infor">
                ...
                <div class="infor-box">
                    <span>预算总费用 </span>
                    <p>{{dataObj.totalBudget}} </p>
                </div>
                <div class="infor-box" v-if="dataObj.hotelBudget">
                    <span>住宿预算 </span>
                    <p>{{dataObj.hotelBudget}} </p>
                </div>
                <div class="infor-box" v-if="dataObj.foodBudget">
                    <span>餐饮预算 </span>
                    <p>{{dataObj.foodBudget}} </p>
                </div>
                <div class="infor-box" v-if="dataObj.otherBudget">
                    <span>其他预算 </span>
                    <p>{{dataObj.otherBudget}} </p>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        ...mapMutations(['change_man','approver_man']),
            refuse:function(){
                 this.$router.push({path:'/opinion',query:{id:this.dataObj.receptionApplyId,typeName:'reception',applyType:10,color:'#fd545c'}})
            },
            ...
            deliverTo(){ //转交
                ...
                this.$router.push({path:'/imchoices',query:{id:this.dataObj.receptionApplyId,receiverIds:newCopy,careOf:true,typeName:'reception',applyType:10,bgcolor:'#fd545c',auditerIds:newApprStr,num:1}})
            },
            approveBack(){ //退回
                 this.$router.push({path:'/approveBack',query:{id:this.dataObj.receptionApplyId,typeName:'reception',applyType:10,color:'#fd545c'}})
            },
            consent:function(type){
                ...
                params={id:this.dataObj.receptionApplyId,receiverIds,auditerIds,receiverCompanyId,auditCompanyId,
                color:'#fd545c',applyType:10,typeName:'reception',pageType:type,careOf:true,num:1}
            ...
            resubmit(){ //再次提交
                this.$router.replace({path:'/reception',query:{receptionId:this.dataObj.receptionApplyId,resubmit:1}})
            },
            urge(){ //催办
                this.isBackout=false
                let that = this;
                this.axios.post('/work/audit'+this.Service.queryString({
                    applyId:this.dataObj.receptionApplyId,
                    type:6,
                    applyType:10,
                })).then(function(res){
                ...
                this.axios.post('/work/audit'+this.Service.queryString({
                    ...
                    applyType:10,
                        ...
                        if(res.data.h.code!=200){
                            ...
                            that.$toast('撤销成功！')
                            setTimeout(()=>{
                                window.location.href = "epipe://?&mark=receptionDetails&_id="+that.dataObj.receptionApplyId+'&data='+JSON.stringify({text:1});;
                            },500)     
                        } 
                    })
            },
            ...
            go_imchoice:function(num){
                this.$router.push({path: 'imchoices', query: {bgcolor:'#fd545c',num:num}})
            },
        ...
        mounted:function(){
            ...
            let that = this;
            this.receptionId = this.$route.query.receptionId;
            let pushId = this.$route.query.pushId
            this.axios.get('/work/reception/info?receptionApplyId='+this.receptionId+'&pushId='+pushId).then(function(res){
                that.dataObj = res.data.b;
                let arr=[];
                that.accessory = that.accessoryFors(that.dataObj.accessory)
                that.title = that.dataObj.username+'的接待申请'
