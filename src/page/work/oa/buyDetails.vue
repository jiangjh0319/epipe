    ...
    <section class="reimburse">
        ...
        <div class="content">
            ...
            <div v-for="(item,index) in dataObj.list" :key="index">
                <div class="reimburseCount">采购明细 {{index+1}}</div>
...
<script>
    ...
    export default {
        ...
        ...mapMutations(['change_man','approver_man']),
            refuse:function(){
                 this.$router.push({path:'/opinion',query:{id:this.dataObj.userBuyId,typeName:'buy',applyType:20,color:'#0fc37c'}})
            },
            ...
            deliverTo(){ //转交
                ...
                this.$router.push({path:'/imchoices',query:{id:this.dataObj.userBuyId,receiverIds:newCopy,careOf:true,typeName:'buy',applyType:20,bgcolor:'#0fc37c',auditerIds:newApprStr,num:1}})
            },
            approveBack(){ //退回
                 this.$router.push({path:'/approveBack',query:{id:this.dataObj.userBuyId,typeName:'buy',applyType:20,color:'#0fc37c'}})
            },
            consent:function(){
              let that = this;
               let copyStr =  this.appAndCopy(this.newCopy)
               let apprStr = this.appAndCopy(this.newAppr,'auditUserId')
                this.$router.push({path:'/opinion',query:{id:this.dataObj.userBuyId,receiverIds:copyStr,auditerIds:apprStr,color:'#0fc37c',typeName:'buy',applyType:20,pageType:'consent'}})
            },
            resubmit(){ //再次提交
                this.$router.replace({path:'/buy',query:{buyId:this.dataObj.userBuyId,resubmit:1}})
            },
            urge(){ //催办
                this.isBackout = false;
                let that = this;
                this.axios.post('/work/audit'+this.Service.queryString({
                        applyId:this.dataObj.userBuyId,
                        type:6,
                        applyType:20,
                })).then(function(res){
                ...
                this.axios.post('/work/audit'+this.Service.queryString({
                    ...
                    applyType:20,
                        ...
                        if(res.data.h.code!=200){
                            ...
                            that.$toast('撤销成功！')
                            setTimeout(()=>{
                                window.location.href = "epipe://?&mark=buyDetails&_id="+that.dataObj.userBuyId+'&data='+JSON.stringify({text:1})
                            },500)     
                        } 
                    })
            },
            goAddress:function(index){ //公出地点跳转
                 this.$router.push({path:'/addressMap',query:{
                     lon:this.dataObj.buyList[index].lon,
                     lat:this.dataObj.buyList[index].lat,
                     outsideAddress:this.dataObj.buyList[index].destination,
                     detailAdress:this.dataObj.buyList[index].detailAddress,
                     bgColor:'#0fc37c'
                     }})
            },
        ...
        mounted:function(){
            ...
            this.axios.get('/work/buy/info?userBuyApplyId='+this.buyId+'&pushId='+pusthId).then(function(res){
                that.dataObj = res.data.b;
                let arr=[];
                that.accessory = that.accessoryFors(that.dataObj.accessory)
                that.title = that.dataObj.username+'的采购申请'
                that.dataObj.list.forEach(item=>{
                    that.total+= (parseFloat(item.number) * parseFloat(item.price))
                })
